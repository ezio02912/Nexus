...
{
    // Lenient formats: only enforced when the user actually provides a value.
    ...
    private Task ShowCreateAsync(IEnumerable<CustomerDto> _)
    {
        ...
        // Pre-fill a suggested, editable code so the user normally doesn't have to think about it.
        _createModel = new CustomerFormModel { Code = $"KH-{DateTime.Now:yyyyMMddHHmmss}" };
    ...
    private async Task SaveAsync()
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(_editModel.Email) && !EmailRegex.IsMatch(_editModel.Email.Trim()))
            {
                await ShowErrorAsync(new InvalidOperationException("Email không đúng định dạng."));
                return;
            }

            if (!string.IsNullOrWhiteSpace(_editModel.Phone) && !PhoneRegex.IsMatch(_editModel.Phone.Trim()))
            {
                await ShowErrorAsync(new InvalidOperationException("Số điện thoại không đúng định dạng."));
                return;
            }
            ...
            if (_isCreate)
            {
                ...
                // Code is optional in the UI; fall back to an auto-generated one (backend requires it).
                var code = string.IsNullOrWhiteSpace(_createModel.Code)
