...
public partial class CrmOpportunities
{
    ...
    // List/Kanban share one page; only the active view is rendered.
    private ViewMode _viewMode = ViewMode.List;
    private bool _boardLoading;
    private List<OpportunityDto> _boardItems = [];
    ...
    private Color ViewColor(ViewMode mode) => _viewMode == mode ? Color.Primary : Color.Secondary;
    ...
    private async Task LoadBoardAsync()
    {
        ...
        try
        {
            ...
            _boardItems = result?.Items.ToList() ?? [];
        }
        catch (Exception ex)
        {
            await ShowErrorAsync(ex);
    ...
    private IEnumerable<OpportunityDto> ItemsForStage(OpportunityStage stage) =>
        _boardItems.Where(x => x.Stage == stage);
