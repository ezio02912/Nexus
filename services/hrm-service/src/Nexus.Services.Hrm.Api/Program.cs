...
MapCrud<EmployeeAllowance>(app, "/api/hrm/allowances", x => $"{x.AllowanceType} {x.Name} {x.Status}");
MapCrud<EmployeeBenefit>(app, "/api/hrm/benefits", x => $"{x.BenefitType} {x.Name} {x.PolicyCode} {x.Status}");
MapCrud<EmployeeDocument>(app, "/api/hrm/documents", x => $"{x.DocumentType} {x.FileName} {x.Status}");
